# Pantheon Frontend Build Spec FULL v4 (en-US)
...
**Version**：2026-05-05-C  
**Scope**：回應 `spec-gap-2026-05-05-C.md` 的 78 條 deeper gaps。  
**Status**：Normative addendum。Pack C 的定義優先於 v3、Pack A、Pack B；若有衝突，以本文件為準。  
**Purpose**: This addendum removes implementation guesswork for Lovable, frontend, and BFF teams. It is the implementation authority for C001–C078.

## 0. Coverage Overview

| Severity | Count | Resolution |
|---|---:|---|
| High | 14 | 本文件fully specified for implementation |
| Medium | 38 | 本文件fully specified with fields, workflows, or UI rules |
| Low | 26 | 本文件fully specified with consistency decisions or future-work rulings |
| Total | 78 | C001–C078 全部 resolved |

---

## 1. Spec Governance and Legacy → v3 Conversion（C001–C005）

### C001 — Legacy → v3 Mapping Table

The BFF must apply the following conversion in the type bridge. Pages must not interpret legacy fields independently.
...
### C002 — Dual-write and apiVersion
...
All removed, merged, or moved tabs must be listed in the migration table.
...
## 2. State Machine Completeness（C006–C012）
...
Authorization:`admin` only；若 target 是 deployment/runtime/capital，必須加 `risk_officer` two-man approval。
...
## 3. Permission Matrix and ActionDescriptor（C013–C018）

### C013 — 完整 role × entity × action matrix

The following table completes the permission coverage for 11 entity groups.`approval=Y` 表示 action 必須至少走 approval 或 high-risk confirmation；若同時在 high-risk catalog，必須使用 confirm token。
...
Ordering:`group` 順序為 primary → secondary → destructive；同 group 依 `order` 升冪；destructive 一律靠右或 dropdown 最底部。
...
Trigger conditions：`incident.severity in ['high','critical']` 或 `runtime.status='degraded'`。It must not be used for routine rebalance or normal promotion.
...
## 4. High-Risk Actions and Confirm Token（C019–C023）
...
## 13. Implementation-Specific Gaps（C071–C078）
...
# Pantheon Frontend Build Spec FULL v2 — en-US

**Version**: v2.0  
**Generated on**: 2026-05-05  
**Purpose**: Complete SA/SD build specification for Lovable and the frontend team.  
**Major correction**: The gaps G01–G92 from `Pantheon Spec Gap Audit — 2026-05-05-A` have been remediated as a normative addendum.  

## v2 Usage Rules

1. The **SA/SD Gap Remediation Pack 2026-05-05-A** in this document is the highest-priority specification source.
2. If Parts 1–8 conflict with the remediation pack, the remediation pack wins.
3. Lovable must implement the remediation pack first: enums, permission truth tables, high-risk actions, confirm tokens, availableActions, state machines, and BFF contracts before visual page polish.
4. SA/SD gaps must not be filled with “etc.”, “and so on”, or reasonable inference. Every action, enum, role, endpoint, payload, status, and workflow step must be explicitly listed.
5. The frontend may use a mock BFF first, but mock data must conform to v2 canonical DTOs and state machines.

---

# Part 9 — SA/SD Gap Remediation Pack 2026-05-05-A

> This chapter is a v2 normative addendum and overrides Parts 1–8.
