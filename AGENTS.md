...
- FE: `https://app.dev.mvl-cap.tw`
- BFF: `https://api.dev.mvl-cap.tw`

Dev builds should use:

```sh
VITE_BFF_MODE=live
VITE_BFF_BASE_URL=https://api.dev.mvl-cap.tw
...
- deployment evidence: `https://app.dev.mvl-cap.tw/deployment.json`

The active VM is `pantheon-dev-deploy` in project `pantheon-dev-20260902`,
zone `asia-east1-b`, with static IP `34.81.52.222`. The prior
`pantheon-lupin-dev-20260719` and `pantheon-benjamin-20260528` projects and
their sslip.io hostnames are retired and must not be probed or used as
fallbacks.
