...
const frontendHandoffRel = "docs/contracts/agora/frontend-generation-output.v1_13.json";
...
const bindHandoffMode = args.has("--bind-handoff");
...
// A changing contract must name a real source commit, not a historical label
// whose generated bytes happened to match an earlier release.
export function assertGitBoundFiles(root, commit, fileHashes) {
  ...
  for (const [rel, expected] of Object.entries(fileHashes)) {
    let actual;
    try {
      actual = sha256Bytes(gitBytes(root, ["show", `${commit}:${rel}`]));
    } catch {
      throw new Error(`Generation source file missing at ${commit}: ${rel}`);
    }
    if (actual !== expected) {
      throw new Error(`Generation source Git bytes mismatch at ${commit}: ${rel}`);
    }
  }
}

function generatedFileHashes(root) {
  return Object.fromEntries(generatedOutputPaths.map((rel) => [rel, sha256File(path.join(root, rel))]));
}

function generatedDigest(hashes) {
  return sha256Bytes(Buffer.from(
    generatedOutputPaths.map((rel) => `${rel}\t${hashes[rel]}\n`).join(""), "utf8",
  ));
}

export function validateFrontendHandoff(root, handoff, contract) {
  if (handoff.contract_family !== "agora.v1.13" || handoff.generated !== true
    || handoff.frontend?.repo !== "ajoe734/execute-plans") {
    throw new Error("Frontend generation handoff identity is invalid");
  }
  for (const key of ["generated_from_contract_commit", "bundle_index_sha256", "openapi_sha256"]) {
    if (handoff.frontend?.[key] !== contract[key]) {
      throw new Error(`Frontend handoff ${key} mismatch`);
    }
  }
  const metadata = handoff.generation_metadata;
  if (metadata?.file_hash_algorithm !== "sha256-exact-git-bytes-v1"
    || metadata?.generated_types_hash_algorithm !== "sha256-path-tab-filehash-lf-v1"
    || JSON.stringify(metadata?.expected_output_paths) !== JSON.stringify(generatedOutputPaths)) {
    throw new Error("Frontend generation handoff algorithms or output paths mismatch");
  }
  const hashes = generatedFileHashes(root);
  if (handoff.frontend?.generated_types_sha256 !== generatedDigest(hashes)) {
    throw new Error("Frontend handoff generated_types_sha256 mismatch");
  }
  assertGitBoundFiles(root, handoff.frontend.runtime_commit, hashes);
}

export function buildFrontendHandoff(root, contract) {
  const runtimeCommit = gitText(root, ["rev-parse", "HEAD"]);
  const hashes = generatedFileHashes(root);
  // Binding happens after committing the generated output. This avoids both a
  // circular self-reference and attributing new output to an older runtime.
  assertGitBoundFiles(root, runtimeCommit, hashes);
  return {
    handoff_version: "1.0",
    ...
    frontend: {
      ...
      ...contract,
      generated_types_sha256: generatedDigest(hashes),
    },
    generation_metadata: {
      expected_output_paths: generatedOutputPaths,
      file_hash_algorithm: "sha256-exact-git-bytes-v1",
      generated_types_hash_algorithm: "sha256-path-tab-filehash-lf-v1",
    },
    generated_at: gitText(root, ["show", "-s", "--format=%cI", runtimeCommit]),
  };
}
...
function loadBundle(pantheonRoot) {
  ...
  const expectedContractCommit = handoff.backend?.contract_commit;
  // Include inherited schemas and bundle indices: the top-level bundle hash
  // alone does not identify a changed schema in an older bundle generation.
  const sourceHashes = Object.fromEntries([
    ...chain.map(({ rel }) => [rel, sha256File(path.join(pantheonRoot, rel))]),
    ...Object.entries(files).map(([rel, hash]) => [`services/control-plane/${rel}`, hash]),
  ]);
  assertGitBoundFiles(pantheonRoot, expectedContractCommit, sourceHashes);
  ...
  return {
    generationContract: {
      generated_from_contract_commit: expectedContractCommit,
      bundle_index_sha256: actualBundleSha,
      openapi_sha256: actualOpenApiSha,
    },
    requestedBundleRel,
...
function main() {
  if (writeMode && bindHandoffMode) {
    fail("Generate and commit output with --write before using --bind-handoff");
  }
  ...
  if (writeMode) {
    ...
    console.log("[contract-drift] Commit generated output, then run this script with --bind-handoff.");
    return;
  }
  ...
  const feHandoffPath = path.join(repoRoot, frontendHandoffRel);
  if (bindHandoffMode) {
    const handoff = buildFrontendHandoff(repoRoot, bundle.generationContract);
    fs.writeFileSync(feHandoffPath, `${JSON.stringify(handoff, null, 2)}\n`, "utf8");
    console.log(`[contract-drift] Bound generated output to ${handoff.frontend.runtime_commit}`);
    return;
  }
  validateFrontendHandoff(repoRoot, readJson(feHandoffPath), bundle.generationContract);
