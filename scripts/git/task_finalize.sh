...
# helper. If scripts/git/task_review_merge_gate.py exists it resolves the
# policy; this repo does not carry that script today, so the gate fails
# closed to the safe default below instead of always granting merge_then_review:
#
...
#     reviewer approves the exact PR head before anyone merges it.
#
#   merge_then_review -- only when the canonical row declares it and requires
#     no independent review. `--auto --merge` is enabled so the dev branch
#     protection's required status checks (Commit trailers / Generated files
#     guard / Component merge gate) turn green and complete the PR alone.
#
# Do not run the task's governed `done` command until GitHub reports the PR
# merged.
...
if [[ "$MERGE_POLICY" != "merge_then_review" ]]; then
  ...
  echo "  next: assigned reviewer approves this exact head, then merges it"
  echo "        (PR #${PR_NUMBER:-<pr-number>}, head $HEAD_SHA)"
fi
