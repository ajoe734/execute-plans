...
TMP_ROOT="$(mktemp -d -t execute-plans-nightly-publish-test-XXXXXX)"
...
mkdir -p "$SEED_DIR/scripts/git" "$SEED_DIR/.github"
cp "$ROOT_DIR/scripts/git/nightly_publish.sh" "$SEED_DIR/scripts/git/nightly_publish.sh"
cp "$ROOT_DIR/.github/branch-workflow.json" "$SEED_DIR/.github/branch-workflow.json"

git -C "$SEED_DIR" add \
  scripts/git/nightly_publish.sh \
  .github/branch-workflow.json
...
if grep -Fq "gh workflow run" "$ROOT_DIR/.github/workflows/nightly-publish-cut.yml"; then
  echo "execute-plans publish cut must not dispatch a deployment" >&2
  exit 1
fi
