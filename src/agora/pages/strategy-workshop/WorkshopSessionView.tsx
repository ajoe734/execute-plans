...
export interface WorkshopSessionViewProps {
  workshopId: string;
...
export function WorkshopSessionView({ governedProposalId, workshopId, onAddToTradingRoom, entry }: WorkshopSessionViewProps): JSX.Element {
  ...
  const [messageReceiptState, setMessageReceiptState] = useState<"none" | "accepted" | "processing" | "succeeded" | "failed">("none");
  const [personaReceiptState, setPersonaReceiptState] = useState<"none" | "submitting" | "succeeded" | "failed" | "degraded">("none");
  ...
  // Initial data load — performs exactly one canonical Workshop projection GET
  useEffect(() => {
    let cancelled = false;
  ...
  // SSE stream subscription — does not reconnect on every event
  useEffect(() => {
    ...
    return teardown;
  }, [workshopId, refreshCards, refreshCompleteness, refreshDailyInteractions, refreshEvents, refreshReadiness, refreshStrategySpec]);
  ...
  const handleSend = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        setPersonaReceiptState("submitting");
        ...
        setPersonaReceiptState("succeeded");
        setComposerValue("");
        refreshCards();
        refreshEvents();
        refreshCompleteness();
        ...
        if (!messageDurablyPosted) {
          setMessageReceiptState("failed");
        } else {
          setPersonaReceiptState("failed");
        }
    ...
    >
      ...
      >
        ...
        <div data-testid="servant-composer" className="border-t border-slate-200 bg-white p-4 shrink-0 flex flex-col gap-3">
          ...
          {/* Distinct truthful receipts */}
          <div className="flex flex-wrap items-center gap-2 shrink-0">
            {/* Durable Workshop message receipt state indicator */}
            {messageReceiptState !== "none" && (
              <div
                className={cn(
                  ...
                  messageReceiptState === "failed" && "bg-red-50 text-red-700 border border-red-200",
                )}
            ...
            {/* Persona interaction receipt indicator */}
            {personaReceiptState !== "none" && (
              <div
                className={cn(
                  "inline-flex items-center gap-1.5 rounded px-2.5 py-1 text-xs font-semibold shrink-0",
                  personaReceiptState === "submitting" && "bg-indigo-50 text-indigo-700 border border-indigo-200 animate-pulse",
                  personaReceiptState === "succeeded" && "bg-green-50 text-green-700 border border-green-200",
                  personaReceiptState === "degraded" && "bg-amber-50 text-amber-700 border border-amber-200",
                  personaReceiptState === "failed" && "bg-red-50 text-red-700 border border-red-200",
                )}
                data-persona-receipt={personaReceiptState}
                data-testid="persona-receipt-state"
              >
                <span>Persona: {personaReceiptState}</span>
              </div>
            )}

            {/* Strategy reconstruction receipt indicator */}
