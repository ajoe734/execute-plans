...
export const BlockersList = ({ blockers }: { blockers: ReadinessBlocker[] }) => (
  <Card className="p-4">
    <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Blockers</h2>
    {blockers.length === 0 ? (
      <p className="mt-3 text-xs text-muted-foreground">No active blockers.</p>
    ) : (
        ...
        {blockers.map((b) => (
          <li key={b.id} className="rounded-md border border-border p-2">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Required role: <span className="text-foreground">{b.requiredRole}</span>
              {" · "}Next action: <span className="text-foreground">{b.nextAction}</span>
            </p>
            {b.linkedEvidence.length > 0 && (
              <p className="mt-1 text-xs text-muted-foreground">
                Evidence: {b.linkedEvidence.map((e) => <span key={e} className="font-mono mr-2">{e}</span>)}
              </p>
            )}
          </li>
        ))}
