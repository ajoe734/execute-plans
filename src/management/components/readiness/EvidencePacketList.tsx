...
export const EvidencePacketList = ({ packets }: { packets: ReadinessPacket[] }) => {
  ...
  return (
    <Card className="p-4">
      <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">{t("mgmt.readiness.evidencePackets")}</h2>
      {packets.length === 0 ? (
        <p className="mt-3 text-xs text-muted-foreground">{t("mgmt.readiness.noPackets")}</p>
      ) : (
