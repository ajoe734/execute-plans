...
const fresh = { read_state: "formal" as const, warnings: [], freshness: { snapshot_at: new Date().toISOString(), materializer_revision: 7, sources: {} } };
const scenarios = [
...
const rows = scenarios.map(([journey_id, status, current_stage, flags]) => ({ journey_id, status, current_stage, flags: [...flags], environment: "paper" as const, severity: "warning", symbol: "2330", updated_at: new Date().toISOString() }));
...
describe("Trade Journeys workbench", () => {
  ...
  it("renders happy, risk reject, broker reject, partial fill, and recon mismatch rows", async () => {
    ...
    renderList();
    for (const [id, status] of scenarios) expect(await screen.findByText(id)).toBeInTheDocument(), expect(screen.getByText(status)).toBeInTheDocument();
  ...
  it("announces degraded, incomplete, stale, and missing-stage truth", async () => {
    ...
    vi.mocked(api.getTradeJourney).mockResolvedValue({ data: { ...rows[4], read_state: "partial", completeness: { missing_stages: ["ledger_booking"] }, revision: 3 }, meta });
