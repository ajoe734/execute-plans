...
export function TradeJourneysPage() {
  ...
  const returnLabel = params.get("return_label") || undefined;
  const detailContext = useMemo(() => { const next = new URLSearchParams(); next.set("tenant_id", tenant); next.set("environment", environment); if (returnTo) next.set("return_to", returnTo); if (returnLabel) next.set("return_label", returnLabel); return next.toString(); }, [tenant, environment, returnTo, returnLabel]);
  ...
  const resolve = async () => { const q = params.get("resolve")?.trim(); if (!q) return; setResolving(true); setError(""); try { const { data } = await resolveTradeJourney({ q, tenant_id: tenant, environment }); if (data.journey_ids.length === 1) { const id = data.journey_ids[0]; navigate(`/management/trade-journeys/${encodeURIComponent(id)}?${detailContext}`); } else if (data.ambiguous || data.journey_ids.length > 1) setError(`Ambiguous identifier: ${data.journey_ids.join(", ")}`); else setError("No journey matched that identifier."); } catch (e) { setError((e as Error).message); } finally { setResolving(false); } };
  return <section className="mx-auto w-full max-w-[1500px] space-y-4 p-4 md:p-6">
    <header>{returnTo && <Link className="mb-1 inline-flex items-center gap-1 text-sm text-primary hover:underline" to={returnTo}><ArrowLeft size={14}/>Back to {returnLabel || "origin"}</Link>}<p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Execution observability</p><div className="flex items-center gap-3"><h1 className="text-2xl font-semibold">Trade Journeys</h1><span role="status" className={`rounded-full px-2 py-1 text-xs ${liveState === "live" ? "bg-emerald-500/10 text-emerald-700" : "bg-amber-500/10 text-amber-700"}`}>{liveState === "live" ? "Live" : liveState === "stale" ? "Live updates stale" : "Connecting"}</span></div><p className="text-sm text-muted-foreground">Canonical signal-to-reconciliation truth. No browser-side domain joins.</p></header>
    {hasFocus && <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-primary/30 bg-primary/5 p-3 text-sm">
      ...
      <Link className="text-primary hover:underline" to={`/management/trade-journeys?${detailContext}`}>Show all journeys</Link>
    </div>}
...
export function TradeJourneyDetailPage() {
  ...
  const returnTo = params.get("return_to") || undefined, returnLabel = params.get("return_label") || undefined;
  const listContext = useMemo(() => { const next = new URLSearchParams(); next.set("tenant_id", tenant); next.set("environment", environment); if (returnTo) next.set("return_to", returnTo); if (returnLabel) next.set("return_label", returnLabel); return next.toString(); }, [tenant, environment, returnTo, returnLabel]);
  ...
  return <section className="mx-auto w-full max-w-[1400px] space-y-5 p-4 md:p-6"><div className="flex flex-wrap items-center gap-4"><Link className="inline-flex items-center gap-1 text-sm text-primary" to={`/management/trade-journeys?${listContext}`}><ArrowLeft size={15}/>All journeys</Link>{returnTo && <Link className="inline-flex items-center gap-1 text-sm text-primary hover:underline" to={returnTo}><ArrowLeft size={15}/>Back to {returnLabel || "origin"}</Link>}</div>
