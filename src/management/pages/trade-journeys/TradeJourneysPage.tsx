...
export function TradeJourneyDetailPage() {
  ...
  return <section className="mx-auto w-full max-w-[1400px] space-y-5 p-4 md:p-6"><div className="flex flex-wrap items-center gap-4"><Link className="inline-flex items-center gap-1 text-sm text-primary" to={`/management/trade-journeys?tenant_id=${encodeURIComponent(tenant)}&environment=${encodeURIComponent(environment)}`}><ArrowLeft size={15}/>All journeys</Link>{returnTo && <Link className="inline-flex items-center gap-1 text-sm text-primary hover:underline" to={returnTo}><ArrowLeft size={15}/>Back to {returnLabel || "origin"}</Link>}</div>
