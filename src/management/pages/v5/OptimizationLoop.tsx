...
export const OptimizationLoopPage = () => {
  ...
  const runs = useV5Live(() => v5.loops.list("optimization"), [], { cacheKey: "v5.loops.optimization" });
    ...
    <>
      ...
      <PageBody>
        <div className="flex flex-col gap-6">
          <div className="order-2 grid gap-3 sm:grid-cols-2 md:order-1 lg:grid-cols-4">
          ...
          <Card className="order-1 p-0 md:order-2">
            <div ref={runsRef} className="scroll-mt-56 px-4 py-3 border-b border-border md:scroll-mt-28">
            ...
            <ManagementTableScroll minScrollWidth={1120}>
              <table className="w-full min-w-[1120px] text-sm">
                <thead ref={approvalRef} className="scroll-mt-56 text-xs text-muted-foreground bg-muted/40 md:scroll-mt-28">
                  <tr>
                  ...
                  {items.map((r) => {
                    ...
                    return (
                      <tr key={r.id} className="relative z-10 scroll-mt-56 border-t border-border md:scroll-mt-28">
                        ...
                        <td className="px-3 py-2">
                          {nextActionHref
                            ? <Link to={nextActionHref} className="relative z-20 scroll-mt-56 text-primary hover:underline md:scroll-mt-28">{nextActionLabel}</Link>
                            : <span className="text-muted-foreground">{nextActionLabel}</span>}
                        </td>
                        <td className="px-3 py-2">
                          {approvalEv
                            ? <Link to={approvalHref(approvalEv.id)} className="relative z-20 scroll-mt-56 text-mono text-xs hover:underline md:scroll-mt-28" aria-label={`Open approval ${approvalEv.id}`}>{approvalEv.id}</Link>
