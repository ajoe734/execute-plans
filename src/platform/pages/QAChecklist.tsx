...
const SECTIONS: Section[] = [
  ...
  {
    id: "agora",
    title: "§12 — Agora Trading Desk",
    items: [
      { id: "a.trading.default", label: "/agora redirects to the Trading Room tab." },
      { id: "a.trading.room", label: "Trading Room shows live trading context, positions, execution, alerts, and decision cards." },
      { id: "a.strategy.workshop", label: "Strategy Workshop shows workshop sessions, conversation cards, readiness, and completeness." },
      { id: "a.strategy.performance", label: "Strategy Performance tab is mounted as the third Trading Desk section." },
      { id: "a.servant", label: "Servant drawer opens from the command bar and keeps workshop context." },
      { id: "a.bottom", label: "Bottom strip exposes jobs, shadow state, and journal drawers without changing tabs." },
      { id: "a.legacy.redirect", label: "Retired Agora paths redirect into the Trading Desk." },
  ...
  },
  {
    ...
    items: [
      { id: "f.split", label: "App clearly separates Management Console and Agora Trading Desk." },
      { id: "f.mc", label: "Management Console combines management, monitoring, response, approval, deployment, rollback, audit." },
      { id: "f.agora", label: "Agora Trading Desk is friendly to analysts and traders, not an admin dashboard." },
...
export const QAChecklist = () => {
  ...
  const handleResetPersist = async () => {
    const { clearPersisted } = await import("@/lib/bff/persistence");
    clearPersisted();
    toast.success(t("qa.persistResetDone"));
    setTimeout(() => window.location.reload(), 400);
  };
  const handleSnapshotNow = async () => {
    const { persistNow } = await import("@/lib/bff/persistence");
    persistNow();
