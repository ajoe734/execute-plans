...
const SCRIPT_PATH = path.resolve(process.cwd(), "scripts/release-candidate.mjs");
const temporaryRoots: string[] = [];

function temporaryRoot() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "release-candidate-test-"));
...
function makeDist(root: string, additions: Record<string, string> = {}) {
  ...
  writeFile(dist, "index.html", "<!doctype html><link rel=\"stylesheet\" href=\"/assets/app.css\"><script type=\"module\" src=\"/assets/app.js\"></script>");
  writeFile(dist, "assets/app.js", "globalThis.__PANTHEON_RELEASE__ = 'safe';\n");
  writeFile(dist, "assets/app.css", "body { color: #111; }\n");
  writeFile(dist, "assets/config.json", `${JSON.stringify({ mode: "live" })}\n`);
...
describe("release candidate preparation and verification", () => {
  it("produces a deterministic canonical digest and strict deployment aliases", () => {
    ...
    expect(fs.readFileSync(path.join(firstDir, "candidate.json"), "utf8"))
      .toBe(fs.readFileSync(path.join(secondDir, "candidate.json"), "utf8"));
    expect(fs.readFileSync(path.join(firstDir, "dist", "deployment.json"), "utf8"))
      .toBe(fs.readFileSync(path.join(secondDir, "dist", "deployment.json"), "utf8"));

    const candidate = JSON.parse(fs.readFileSync(path.join(firstDir, "candidate.json"), "utf8"));
    const deployment = JSON.parse(fs.readFileSync(path.join(firstDir, "dist", "deployment.json"), "utf8"));
    ...
    expect(candidate.files).not.toContainEqual(expect.objectContaining({ path: "deployment.json" }));
    ...
    expect(verify(firstDir).artifactDigestSha256).toBe(first.artifactDigestSha256);
  });
  ...
  it("supports prepare and verify through the CLI", () => {
    ...
    const verified = cli([
      ...
      "--expected-frontend-sha", FRONTEND_SHA,
      "--expected-gate-run-id", GATE_RUN_ID,
      "--expected-bff-sha", BFF_SHA,
      "--expected-bff-base-url", BFF_BASE_URL,
  ...
  it("rejects asset tampering and digest alias tampering", () => {
    ...
    fs.appendFileSync(path.join(candidateDir, "dist", "assets", "app.js"), "// tampered\n");
    expect(() => verify(candidateDir)).toThrow(/asset manifest does not match/u);
  ...
  it("rejects the wrong expected frontend SHA or gate run", () => {
    ...
    expect(() => verify(candidateDir, { expectedFrontendSha: OTHER_FRONTEND_SHA }))
      .toThrow(/frontend SHA does not match/u);
    expect(() => verify(candidateDir, { expectedGateRunId: "999999" }))
      .toThrow(/gate run ID does not match/u);
  });
  ...
  it("rejects unsafe build flags during prepare and verify", () => {
    ...
    fs.writeFileSync(candidatePath, `${JSON.stringify(candidate, null, 2)}\n`);
    expect(() => verify(candidateDir)).toThrow(/VITE_BFF_ALLOW_DEV_STUB_WRITES must be false/u);
  });

  it.each([
    ["bearer", "globalThis.auth = 'Bearer top-secret-token-123';\n", /bearer credential/u],
    ["client-secret", "const config = { client_secret: 'client-secret-value' };\n", /client secret/u],
    ["private-key", "const key = `-----BEGIN PRIVATE KEY-----\\nabc123\\n-----END PRIVATE KEY-----`;\n", /private key/u],
    ["service-role", "const config = { service_role_key: 'service-role-secret' };\n", /service-role credential/u],
    ["token-sentinel", "const token = 'release-token-sentinel-value';\n", /token sentinel/u],
  ])("rejects a generic %s credential without reflecting its value", (_name, source, expectedError) => {
    const root = temporaryRoot();
  ...
  it("rejects an exact configured secret sentinel without echoing it", () => {
    ...
    const root = temporaryRoot();
    const distDir = makeDist(root, { "assets/leak.css": `.x::after { content: "${sentinel}"; }\n` });
    let message = "";
    try {
  ...
  it("rejects symlinks and canonical-manifest path escapes", () => {
    const root = temporaryRoot();
    const distDir = makeDist(root);
    fs.symlinkSync(path.join(distDir, "assets", "app.js"), path.join(distDir, "assets", "alias.js"));
  ...
  it("fails closed on a non-empty browser bearer environment input", () => {
    ...
    const result = cli([
      "prepare",
      "--dist-dir", distDir,
      "--output-dir", path.join(root, "candidate"),
      "--frontend-sha", FRONTEND_SHA,
      "--bff-sha", BFF_SHA,
      "--gate-run-id", GATE_RUN_ID,
      "--gate-run-url", GATE_RUN_URL,
    ...
    expect(result.stderr).toMatch(/browser bearer environment input must be empty/u);
